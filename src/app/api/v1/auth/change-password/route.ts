...
const schema = z.object({ currentPassword: z.string().min(1).max(128).optional(), newPassword: z.string().min(10).max(128).regex(/[A-Z]/).regex(/[a-z]/).regex(/\d/) });
export async function POST(request: NextRequest) {
  try {
    ...
    if (!account || (account.passwordHash && (!input.currentPassword || !(await compare(input.currentPassword, account.passwordHash)))) || (!account.passwordHash && input.currentPassword)) throw new AppError("INVALID_PASSWORD", "Current password is incorrect", 422);
