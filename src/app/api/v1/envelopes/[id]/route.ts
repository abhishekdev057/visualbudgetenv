...
import { envelopeUpdateSchema, uuid } from "@/lib/validation";
...
export async function PATCH(request: NextRequest, { params }: Context) { try { await assertTrustedOrigin(request); const user = await requireRequestUser(request); const { id } = await params; return ok(await updateEnvelope(user.id, uuid.parse(id), envelopeUpdateSchema.parse(await readJson(request)))); } catch (error) { return apiError(error); } }
